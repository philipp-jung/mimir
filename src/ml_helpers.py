...
def set_binary_cleaning_suggestions(predicted_labels: List[int],
                                    ...
                                    detected_cells: List[Tuple[int,int]]) -> List[Tuple[int,int]]:
    """
    ...
    @return: List of cells for which no error correction has been made by the correction classifier.
    """
    suggestions = {}
    ...
    cells_without_suggestion = set(detected_cells) - set(suggestions.keys())
    ...
    for error_cell in suggestions:
        if len(suggestions[error_cell]) > 1: # multiple suggestions were classified by the correction classifier
            ...
            # take the suggestion with the highest probability
    ...
    return list(cells_without_suggestion)
