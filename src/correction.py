...
class Cleaning:
    ...
    @staticmethod
    def _value_based_models_updater(models, ud):
        """
        ...
        for encoding in ["identity", "unicode"]:
            encoded_old_value = Cleaning._value_encoder(ud["old_value"], encoding)
            if remover_transformation:
                Cleaning._to_model_adder(models[0], encoded_old_value, json.dumps(remover_transformation))
            if adder_transformation:
                Cleaning._to_model_adder(models[1], encoded_old_value, json.dumps(adder_transformation))
            if replacer_transformation:
                Cleaning._to_model_adder(models[2], encoded_old_value, json.dumps(replacer_transformation))
            Cleaning._to_model_adder(models[3], encoded_old_value, ud["new_value"])

    @staticmethod
    def _imputer_based_corrector(model: Dict[int, pd.DataFrame], ed: dict) -> Dict[str, float]:
        """
    ...
    @staticmethod
    def _value_based_corrector(models, ed):
        """
        ...
        for m, model_name in enumerate(["remover", "adder", "replacer", "swapper"]):
            ...
            for encoding in ["identity", "unicode"]:
                results_dictionary = {}
                encoded_value_string = Cleaning._value_encoder(ed["old_value"], encoding)
    ...
    def initialize_models(self, d):
        ...
        d.corrections_features = []  # don't need further processing being used in ensembling.

        for feature in self.FEATURE_GENERATORS:
            if feature == 'vicinity':
                if self.VICINITY_FEATURE_GENERATOR == 'pdep':
                    for o in self.VICINITY_ORDERS:
                        d.corrections_features.append(f'vicinity_{o}')
                    ...
                    for o in self.VICINITY_ORDERS:
                        for lhs in combinations(range(cols), o):
                            d.corrections_features.append(f'vicinity_{o}_{str(lhs)}')
            elif feature == 'value':
                ...
                d.corrections_features.extend(models_names)
            else:
                d.corrections_features.append(feature)

        d.corrections = helpers.Corrections(d.corrections_features)
        d.inferred_corrections = helpers.Corrections(d.corrections_features)
    ...
    @staticmethod
    def _feature_generator_process(d: dataset.Dataset,
                                   error_cell: Tuple[int, int],
                                   is_synth: bool,
                                   feature_generators: List[str],
                                   correction_model_names: List[str],
                                   fd_feature: str,
                                   vicinity_feature_generator: str,
                                   vicinity_orders: List[int],
                                   n_best_pdeps: int,
                                   pdep_features: Tuple[str],
                                   gpdep_threshold: float,
                                   labeling_budget: int
                                   ) -> Dict:
        """
        ...
        """
        llm_requests: List[helpers.LLMRequest] = []
        corrections = helpers.Corrections(correction_model_names)
        inferred_corrections = helpers.Corrections(correction_model_names)
        ...
        if "fd" in feature_generators:
            fd_corrections = pdep.fd_based_corrector(d.fd_inverted_gpdeps, d.fd_counts_dict, error_dictionary, fd_feature)
            if is_synth:
                inferred_corrections.get('fd')[error_cell] = fd_corrections
            else:
                corrections.get('fd')[error_cell] = fd_corrections

        if "vicinity" in feature_generators:
            if vicinity_feature_generator == 'naive':
                for o in vicinity_orders:
                    naive_corrections = pdep.vicinity_based_corrector_order_n(
                        counts_dict=d.vicinity_models[o],
                        ed=error_dictionary)
                    if is_synth:
                        for lhs in naive_corrections:
                            inferred_corrections.get(f'vicinity_{o}_{str(lhs)}')[error_cell] = naive_corrections[lhs]
                    else:
                        for lhs in naive_corrections:
                            corrections.get(f'vicinity_{o}_{str(lhs)}')[error_cell] = naive_corrections[lhs]

            elif vicinity_feature_generator == 'pdep':
                for o in vicinity_orders:
                    pdep_corrections = pdep.pdep_vicinity_based_corrector(
                        inverse_sorted_gpdeps=d.inv_vicinity_gpdeps[o],
                        counts_dict=d.vicinity_models[o],
                        ed=error_dictionary,
                        n_best_pdeps=n_best_pdeps,
                        features_selection=pdep_features,
                        gpdep_threshold=gpdep_threshold)
                    if is_synth:
                        inferred_corrections.get(f'vicinity_{o}')[error_cell] = pdep_corrections
                    else:
                        corrections.get(f'vicinity_{o}')[error_cell] = pdep_corrections
            else:
                raise ValueError(f'Unknown VICINITY_FEATURE_GENERATOR '
                                 f'{vicinity_feature_generator}')

        if 'llm_correction' in feature_generators and not is_synth and len(d.labeled_tuples) == labeling_budget:
            """
            ...
            if error_dictionary['old_value'] != '':  # If there is no value to be transformed, skip.
                ...
                if len(error_correction_pairs) > 0:
                    ...
                    llm_requests.append(helpers.LLMRequest(error_cell, 'llm_correction', prompt))

        if 'llm_master' in feature_generators and len(d.labeled_tuples) == labeling_budget:
            ...
            if not is_synth:
                ...
                if len(rows_without_errors) >= 3:
                    ...
                    llm_requests.append(helpers.LLMRequest(error_cell, 'llm_master', prompt))
            else:  # synth - return empty prompts and use cache to make cleaning suggestions due to API cost.
                # llm_requests.append((error_cell, 'llm_master', ''))
                pass

        if "domain_instance" in feature_generators:
            if is_synth:
                inferred_corrections.get('domain_instance')[error_cell] = Cleaning._domain_based_corrector(d.domain_models, error_dictionary)
            else:
                corrections.get('domain_instance')[error_cell] = Cleaning._domain_based_corrector(d.domain_models, error_dictionary)

        if "value" in feature_generators:
            if not is_synth:
                value_correction_suggestions = Cleaning._value_based_corrector(d.value_models, error_dictionary)
                for model_name, encodings_list in value_correction_suggestions.items():
                    for encoding, correction_suggestions in zip(['identity', 'unicode'], encodings_list):
                        corrections.get(f'value_{model_name}_{encoding}')[error_cell] = correction_suggestions

        if "auto_instance" in feature_generators:
            imputer_corrections = Cleaning._imputer_based_corrector(d.imputer_models, error_dictionary)
            ...
            # a feature.
            if len(d.labeled_tuples) == labeling_budget and len(imputer_corrections) == 0:
                imputer_corrections = {}
            if is_synth:
                inferred_corrections.get('auto_instance')[error_cell] = imputer_corrections
            else:
                corrections.get('auto_instance')[error_cell] = imputer_corrections
        return {'llm_requests': llm_requests,
                'corrections': corrections,
                'inferred_corrections': inferred_corrections}
        ...
        if not synchronous:
            ...
            with concurrent.futures.ProcessPoolExecutor() as executor:
                feature_results = executor.map(self._feature_generator_process,
                                      ...
                                      cell_list,
                                      is_synth_list,
                                      [self.FEATURE_GENERATORS for _ in range(len(d_list))],
                                      [d.corrections_features for _ in range(len(d_list))],
                                      [self.FD_FEATURE for _ in range(len(d_list))],
                                      [self.VICINITY_FEATURE_GENERATOR for _ in range(len(d_list))],
                                      [self.VICINITY_ORDERS for _ in range(len(d_list))],
                                      [self.N_BEST_PDEPS for _ in range(len(d_list))],
            ...
            for (d, cell, is_synth) in process_args_list:
                feature_results.append(self._feature_generator_process(d,
                                                                  cell,
                                                                  is_synth,
                                                                  self.FEATURE_GENERATORS,
                                                                  d.corrections_features,
                                                                  self.FD_FEATURE,
                                                                  self.VICINITY_FEATURE_GENERATOR,
                                                                  self.VICINITY_ORDERS,
                                                                  self.N_BEST_PDEPS,
                                                                  self.PDEP_FEATURES,
                                                                  self.GPDEP_THRESHOLD,
                                                                  self.LABELING_BUDGET,
                                                                  ))
            self.logger.debug('Finish synchronous user feature generation.')

        for result in feature_results:
            llm_requests.extend(result['llm_requests'])

            # move corrections over to Mimir's correction store
            for corrector_name, corrections in result['corrections'].correction_store.items():  
                for cell in corrections:
                    d.corrections.get(corrector_name)[cell] = corrections[cell]
            
            for corrector_name, corrections in result['inferred_corrections'].correction_store.items():
                for cell in corrections:
                    d.inferred_corrections.get(corrector_name)[cell] = corrections

        # generate llm-features
        if len(d.labeled_tuples) == self.LABELING_BUDGET:
            for llm_request in llm_requests:
                correction, token_logprobs, top_logprobs = helpers.fetch_cached_llm(d.name, llm_request.cell, llm_request.prompt, llm_request.corrector_name, d.error_fraction, d.version, d.error_class)
                correction_dicts = helpers.llm_response_to_corrections(correction, token_logprobs, top_logprobs)
                d.corrections.get(llm_request.corrector_name)[llm_request.cell] = correction_dicts

        if self.VERBOSE:
            self.logger.info("Fetched LLM requests. User Features generated.")
    ...
    def generate_inferred_features(self, d, synchronous):
        ...
        if self.SYNTH_TUPLES > 0 and len(ranked_candidate_rows) > 0:
            ...
            llm_requests: List[Tuple[Tuple[int, int], str, Union[str, None]]] = []
    ...
    def run(self, d, random_seed):
        ...
        while len(d.labeled_tuples) <= self.LABELING_BUDGET:
            ...
            self.generate_inferred_features(d, synchronous=False)
